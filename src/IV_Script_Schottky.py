...
def RUN(search_bias, replace_bias):
	...
	# run the code
	cmd_run_sim =	'./Implicit_Schottky'

	failure, output = commands.getstatusoutput(cmd_run_sim)
	if failure:
		raise NameError('Failed to find Implicit_Schottky: Recompile or check loaded modules.')
...
###########:#######################################################
#bias_list = [0.0, 0.1, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55]
bias_list = [0.37, 0.39, 0.41, 0.43]
